document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkResolutionAndDoSomething() {
    if (window.matchMedia("(max-width: 500px)").matches) {
     let isAlive = setInterval(function () {
       let catTop = parseInt(
         window.getComputedStyle(cat).getPropertyValue("top")
       );
       let chairLeft = parseInt(
         window.getComputedStyle(chair).getPropertyValue("left")
       );
  ...
  window.addEventListener("resize", checkResolutionAndDoSomething);

  checkResolutionAndDoSomething();
